 ...
 * @param {{ search: string, faculty: string, birthday: string, studyStart: string }} [params] - Поисковая строка
...
function getStudentList(params = {}) {
    ...
    return students.filter(student => {
        ...
        let facultyFilter = true;
        let birthdayFilter = true;
        let studyStartFilter = true;
        ...
        if (params.search?.length) {
            searchFilter = [
                ...
                .some(str => str.toLowerCase().includes(params.search.trim().toLowerCase()));
        }

        if (params.faculty?.length) {
            facultyFilter = student.faculty.toLowerCase().includes(params.faculty.trim().toLowerCase());
        }

        if (params.birthday?.length) {
            birthdayFilter = student.birthday.toLowerCase().includes(params.birthday.trim().toLowerCase());
        }

        if (params.studyStart?.length) {
            studyStartFilter = student.studyStart.toLowerCase().includes(params.studyStart.trim().toLowerCase());
        }

        return searchFilter && facultyFilter && birthdayFilter && studyStartFilter;
    });
}
